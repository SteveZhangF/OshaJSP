...
public class FormComponentDAOImpl implements FormComponentDAO {
	...
	public String save(FormComponent p) {
		...
		String sql = "insert into form_model.tb_form_component (uuid,form_id,component_name,component_type_id,component_content) select ?,?,?,uuid,? from form_model.tb_form_component_type where form_model.tb_form_component_type.name=?";
		...
		try {
			...
			ps.setString(5, p.getComponent_type());
			ps.setString(4, p.getComponent_content());
			ps.executeUpdate();
			return uuid;
		} catch (Exception e) {
