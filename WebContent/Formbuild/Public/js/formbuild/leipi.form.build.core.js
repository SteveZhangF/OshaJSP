	...
	var LPB = window.LPB = window.LPB
			|| {
				...
				genSource : function() {
					...
					$.each($("#build").find(".component"), function(i, e) {
						var component_id=$(e).attr("component_id");
						if (component_id==null||component_id.length != 32) {
							component_id = getUUID();
							$(e).attr("component_id",component_id);
						}
					});
					
					var form_id=$("#build").find("form").attr("form_id");
					if (form_id==null||form_id.length != 32) {
						form_id = getUUID();
						$("#build").find("form").attr("form_id",form_id);
					}
					
					// scrubbbbbbb
					$.each($($temptxt).find(".component"), function(i, e) {
						$(e).attr({
							"title" : null,
					...
					$("#source").val(
							$temptxt.html().replace(
									/\n\ \ \ \ \ \ \ \ \ \ \ \ /g, "\n"));
				}

			};
	/*
	 * 表单名称控件 form_name acc 是 class="component" 的DIV e 是 class="leipiplugins"
	 * 的控件
	 */
				...
				function(e) {
					...
					$.each(inputs, function(i, e) {
						...
						if (attr_name == 'orgvalue') {
							...
							active_component.find(".leipiplugins-orgvalue")
									.text(attr_val);
						}
						active_component.popover("hide");
						LPB.genSource();// 重置源代码
									...
									function(md) {
										...
										var delayed = setTimeout(
												function() {
													if (type === "main") {
														$temp = $(
																"<form class='form-horizontal span6' id='temp'></form>")
																.append(
																		$this
																				.clone());
													} else {
													...
													var half_box_height = ($temp
															.height() / 2);
													var half_box_width = ($temp
															.width() / 2);
													var $target = $("#target");
													var tar_pos = $target
															.offset();
																		...
																		if (mm_mouseX > tar_pos.left
																			...
																			if (tops.length > 0) {
																				...
																				if ($target_component.length > 0) {
																					$(
																							$target_component[$target_component.length - 1])
																							.css(
																									"border-bottom",
																									"1px solid #22aaff");
																				}
													...
													$("body")
															.delegate(
																	...
																	function(mu) {
																		...
																		if (mu_mouseX
																				+ half_box_width > tar_pos.left
																				&& mu_mouseX
																						- half_box_width < tar_pos.left
																						+ $target
																								.width()
																				&& mu_mouseY
																						+ half_box_height > tar_pos.top
																				&& mu_mouseY
																						- half_box_height < tar_pos.top
																						+ $target
																								.height()) {
					...
					$("#target")
							.delegate(
									...
									function(e) {
										...
										//class="leipiplugins"
										var leipiplugins = active_component
												.find(".leipiplugins"), plugins = $(
												leipiplugins).attr(
